<script setup>
    ...
    const itemtypes_can_create = computed(() => {
        ...
        for (const itemtype in all_itemtypes) {
            if (all_itemtypes[itemtype].allow_create) {
...
<template>
    <div :id="element_id" :class="`kanban-column card ${folded ? 'collapsed' : ''} ${column_data['_protected'] ? 'kanban-protected' : ''}`">
            ...
            <div class="kanban-column-header-content p-2 pb-0">
                <span class="content-left">
                    ...
                    <span :class="`kanban-column-title badge ${column_data.color_class || ''}`" v-text="column_data.name"
                          :style="`${bg_color ? 'background-color:' + bg_color : 'transparent'}; ${text_color ? 'color:' + text_color : ''}`"></span>
                </span>
        ...
        <ul class="kanban-body card-body">
            <template v-if="column_data.drop_only">
                <li class="position-relative mx-auto mt-2" style="width: 250px"
                    v-text="__('This column cannot support showing cards due to how many cards would be shown. You can still drag cards into this column.')">
                </li>
            </template>
