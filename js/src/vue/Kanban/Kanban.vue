<script setup>
    ...
    function updateFoldColumn(e) {
        Object.values(user_state.value.state).find((c) => parseInt(c.column) === parseInt(e.column_id)).folded = e.folded;
        $.ajax({
            type: "POST",
            url: CFG_GLPI.root_doc + '/ajax/kanban.php',
            data: {
                    ...
                    <div class="dropdown-menu kanban-form kanban-add-column-form p-2" role="menu">
                        ...
                        <div class="kanban-item-content">
                            ...
                            <ul class="kanban-columns-list ps-1">
                                <li v-for="(column, column_id) in filtered_columns" :key="column_id">
                                    <input v-if="isColumnVisible(column_id)" type="checkbox" class="form-check-input"
                                           :value="column_id" checked="checked" @change="updateColumnVisibility"/>
                                    <input v-else type="checkbox" class="form-check-input"
                                           :value="column_id" @change="updateColumnVisibility"/>
                                    <span :class="`kanban-color-preview ${column.color_class ? column.color_class : ''}`"
