...
import Clipboard from "../../js/modules/Clipboard.js";
...
describe('Clipboard module', () => {
    ...
    test('Get module', () => {
        expect(GLPI.getModule('clipboard')).toBeInstanceOf(Clipboard);
    });

    test('Global binds', () => {
        const clipboard = GLPI.getModule('clipboard');
        const global_binds = GLPI.getModule('clipboard').getLegacyGlobals();

        Object.entries(global_binds).forEach(([key, value]) => {
            expect(window[key]).toBe(clipboard[value]);
        });
    });
