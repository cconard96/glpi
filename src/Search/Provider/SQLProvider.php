...
final class SQLProvider implements SearchProviderInterface
{
            ...
            case "glpi_users.name":
                if ($itemtype !== User::class) {
                    if ($opt->isForceGroupBy()) {
                        ...
                        if (
                            (($itemtype === Ticket::class) || ($itemtype === Problem::class))
                            && isset($opt['joinparams']['beforejoin']['table'])
                            && in_array($opt['joinparams']['beforejoin']['table'], $itilobject_tables, true)
                        ) { // For tickets_users
    ...
    public static function getDefaultWhereCriteria(string $itemtype): array
    {
        ...
        switch ($itemtype) {
            ...
            case 'Reminder':
                $criteria = \Reminder::getVisibilityCriteria();
                break;
    ...
    public static function getWhereCriteria($nott, $itemtype, $ID, $searchtype, $val, $meta = 0): ?array
    {
         ...
         * @return void
         */
        $append_criterion_with_search = static function(array &$criteria, string $value) use ($SEARCH, $RAW_SEARCH): void {
            if ($RAW_SEARCH !== null) {
        ...
        switch ($inittable . "." . $field) {
            ...
            case "glpi_users.name":
                ...
                if ($itemtype === User::class) { // glpi_users case / not link table
                    ...
                    return [new QueryExpression(self::makeTextCriteria("`$table`.`$field`", $val, $nott, '', true))];
                }
                ...
                $append_criterion_with_search(
                    $criteria[$tmplink],
                    new QueryExpression("CONCAT(`$table`.`$name1`, ' ', `$table`.`$name2`)")
                );
            ...
            switch ($opt["datatype"]) {
                ...
                case "date_delay":
                    ...
                    if ($opt["datatype"] === "date_delay") {
                        ...
                        $date_computation = "ADDDATE(`$table`." . $opt["datafields"][1] . ",
                                               INTERVAL (`$table`." . $opt["datafields"][2] . "
                                                         $add_minus)
                                               $delay_unit)";
                    }
                    ...
                    if (is_numeric($val) && !$decimal_contains) {
                        ...
                        if (isset($opt["width"])) {
                            ...
                            if ($nott) {
                                return [
                                    'OR' => [
                                        [$tocompute => ['<', $numeric_val - $searchopt[$ID]["width"]]],
                                        [$tocompute => ['>', $numeric_val + $searchopt[$ID]["width"]]],
                                    ] + $ADD
                                ];
                            }
    ...
    public static function getDefaultJoinCriteria(string $itemtype, string $ref_table, array &$already_link_tables): array
    {
        ...
        switch ($itemtype) {
            ...
            case 'Reminder':
                $out = \Reminder::getVisibilityCriteria();
                break;

            case 'RSSFeed':
                $out = \RSSFeed::getVisibilityCriteria();
                break;
            ...
            case 'ProjectTask':
                ...
                $out = array_merge($out, self::getLeftJoinCriteria(
                    $itemtype,
                    $ref_table,
                    $already_link_tables,
                        ...
                        $itemtype_join = [
                            'LEFT JOIN' => [
                                "$new_table$AS" => [
                                    'ON' => [
                                        $nt => 'itemtype',
                                        new \QuerySubQuery($used_itemtype),
                                    ]
                                ]
                            ]
                        ];
